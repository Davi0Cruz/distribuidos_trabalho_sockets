...
class PowerSensor:
    ...
    def simulate_power(self):
        """Lida com as mudanças de potência e envia dados periodicamente ao Gateway"""
        while True:
            # Testando se o ar condicionado e a lâmpada estão conectados
            ...
            if self.gateway_ip:
                ...
                sensor_data.sensor_type = "power"
                sensor_data.value = self.state["power"]
    ...
    def handle_command(self, command_msg):
        ...
        try:
            ...
            if command == "GET_STATUS":
                ...
                response.attributes["power"] = str(self.state["power"])
    ...
    def run(self):
        ...
        # Thread para simulação de potência
        sensor_thread = threading.Thread(target=self.simulate_power)
        sensor_thread.daemon = True
        sensor_thread.start()
        
        print(f"Power Sensor running on port {self.TCP_PORT}")
